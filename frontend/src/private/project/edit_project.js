...
class EditProject extends React.Component{
  ...
  render(){
    const isEmpty = this.state.description.trim() === '' || this.state.status === '';
    return(
        <div>
          ...
          >
            <Card style={PRIVATE_PAGE_STYLE.card}>
               <CardContent style={PRIVATE_PAGE_STYLE.cardContent}>
                ...
                />
                <InputLabel
                  style={PRIVATE_PAGE_STYLE.inputLabelStyle}
                >
                  Status:
                </InputLabel>
                <Select
  ...
  _handleEditButtonClick = () =>{
    ...
    REQ_HELPER.putWithoutCooki(`${URL_REPO.BE_EDIT_PROJECT}/${id}`)
      .send({
        ...
        window.location = `${URL_REPO.SHOW_PROJECT}/${this.state.user}`;
      }).catch(err =>{
        console.log(err);
      })
  };
