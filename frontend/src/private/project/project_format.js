...
class ProjectFormat extends React.Component {
          ...
          <CardActions style={PRIVATE_PAGE_STYLE.cardActions}>
            ...
            {cookie.load("userInfo") ? (
              <Tooltip placement="bottom" title="Like">
                <IconButton onClick={this._handleVote}>
                  <FavoriteIcon style={PRIVATE_PAGE_STYLE.voteStyle} />
                </IconButton>
              </Tooltip>
            ) : (
              <Tooltip placement="bottom" title="Like">
            ...
            {id === this.props.userId ? (
              <div>
                ...
                <Tooltip placement="bottom" title="Show To-Do">
                  ...
                  >
                    <ViewListIcon style={PRIVATE_PAGE_STYLE.taskStyle} />
                  </IconButton>
                </Tooltip>
              </div>
            ) : null}
  ...
  _handleEditButtonClick = id => {
    ...
    window.location = `/${userId}${URL_REPO.SHOW_PROJECT}${URL_REPO.EDIT_PROJECT}/${id}`;
  };

  _handleDeleteButtonClick = id => {
    REQ_HELPER.deleteWithCooki(`${URL_REPO.BE_DELETE_PROJECT}/${id}`)
      .then(() => {
        window.location = `/${this.props.userId}${URL_REPO.SHOW_PROJECT}`;
      })
      .catch(err => {
        console.log(err);
      });
  ...
  _handleTaskButtonClick = id => {
    const userId = cookie.load("userInfo").id;
    window.location = `/${userId}${URL_REPO.SHOW_PROJECT}/${id}${URL_REPO.TO_DO_LIST}`;
  };
